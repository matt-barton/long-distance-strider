...
app.get('/:id', isAuth, (req, res, next) => {
    ...
    function renderRacePage (race, allRunners) {
      console.log(allRunners, race);
    ...
    Race.findOne({ _id: req.params.id })
      .exec()
      .then(function (race) {
        return getAllRunners().then(function (runners) {
          renderRacePage(race, runners);
        });
      })
      .catch(next);
});

app.post('/ignore/:id', isAuth, (req, res, next) => {
  try {
    Race.update({ _id: req.params.id }, { $set: { ignore: true }}).exec().then(() => {
      res.sendStatus(200);
    });
  }
  catch (e) {
...
app.post('/process/:id', isAuth, (req, res, next) => {
  try {
    Race.findOne({ _id: req.params.id }).exec().then(race => {
      let newRunners = 0,
        updatedRunners = 0,
...
app.post('/mark-processed/:id', isAuth, (req, res, next) => {
  try {
    Race.findOne({ _id: req.params.id }).exec().then(race => {
      race.processed = true;
